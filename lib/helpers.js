...
var helpers = {
  repeat: function (min, max, options) {
    ...
    if (arguments.length === 3) {
      ...
      if (typeof options.hash.min === 'number' && typeof options.hash.max === 'number') {
        ...
        throw new Error('The repeat helper requires either a single count value or both min and max values');
      }
    } else {
      throw new Error('The repeat helper requires either a single count value or both min and max values');
    }
  ...
  char: function (charset) {
    if (arguments.length !== 2 || typeof charset !== 'string') {
      throw new Error('The char helper requires one string param');
    }
    ...
    if (arguments.length === 1) {
      ...
      if (options.hash.min || options.hash.max) {
        if (typeof options.hash.min === 'number' && typeof options.hash.max === 'number') {
          ...
          throw new Error('The lorem helper requires either a word count or min and max values');
        }
      } else {
        totalWords = 25;
      }
  ...
  random: function () {
    if (arguments.length < 2) {
      throw new Error('The random helper requires at least one param');
    }
  ...
  add: function (a, b) {
    if (arguments.length !== 3 || typeof a !== 'number' || typeof b !== 'number') {
      throw new Error('The add helper requires two number params');
    }
    return a + b;
  },
  ...
  step: function (inc, options) {
    if (arguments.length !== 2 || options.data.index == null) {
      throw new Error('The step helper require a numeric value and can only be used inside #repeat and #each blocks');
    }
