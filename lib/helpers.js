...
function getFirstName (options) {
  ...
  var ret = utils.randomFromArray(options.data.root.firstNames);
  cache.firstName = ret;
...
function getLastName (options) {
  ...
  // Each helper is allowed to use the cached value just once.
  var cache = options.data.root.__cache;
  var ret = utils.randomFromArray(options.data.root.lastNames);
  cache.lastName = ret;
...
function getCompany (options) {
  ...
  // Each helper is allowed to use the cached value just once.
  var cache = options.data.root.__cache;
  var ret = utils.randomFromArray(options.data.root.companies);
  cache.company = ret;
...
var helpers = {
  ...
  zipcode: function () {
    var ret = utils.randomInt(1000, 99999).toString();
    if (ret.length < 5) {
      ret = '0' + ret;
    }
    return ret;
  ...
  postcode: function () {
    var ret = utils.randomChar() + utils.randomChar() + utils.randomInt(0, 9) + ' ' +
  ...
  color: function (options) {
    return utils.randomFromArray(options.data.root.colors);
  },
